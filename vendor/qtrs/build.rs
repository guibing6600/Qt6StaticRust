...
/// Candidate qmake paths: those specified by the QMAKE environment variable take priority (used for static Qt)
fn qmake_candidates() -> Vec<String> {
    if let Ok(qmake) = std::env::var("QMAKE") {
        if !qmake.is_empty() {
            return vec![qmake];
        }
    }
    vec!["qmake6".to_string(), "qmake".to_string(), "qmake-qt5".to_string()]
}

/// Find Qt installation and version
fn find_qt() -> Option<(String, String)> {
    for cmd in &qmake_candidates() {
