...
impl Label {
    ...
    /// Set the label's pixmap from an icon file (SVG/PNG), scaled to `(w, h)`.
    ///
    /// SVG files are rendered through the Qt SVG icon engine, so they work
    /// without the SVG image-format plugin.
    pub fn set_pixmap_from_icon(&self, icon_path: &str, w: i32, h: i32) {
        debug_assert!(!self.ptr.is_null(), "Label::set_pixmap_from_icon on null pointer");
        let_cxx_string!(c_path = icon_path);
        unsafe { ffi::QLabel_setPixmapFromIcon(self.ptr, &c_path, w, h); }
    }
