...
inline void QWidget_setLayoutDirection(QWidget *w, int dir) {
    w->setLayoutDirection(static_cast<Qt::LayoutDirection>(dir));
}
...
// --- Click support for plain QWidget ---
// Plain QWidget has no clicked() signal. RpClickEmitter is an event filter
// that intercepts the mouse press (so this widget becomes the mouse grabber
// and the matching release is delivered here even over child labels), then
// fires the Rust closure on left-button release. The filter object is
// parented to the widget by installEventFilter and dies with it.
class RpClickEmitter : public QObject {
public:
    explicit RpClickEmitter(uint64_t ctx) : QObject(nullptr), m_ctx(ctx) {}
...
protected:
    bool eventFilter(QObject *obj, QEvent *event) override {
        ...
        case QEvent::MouseButtonPress:
            if (w) {
                // Toggle a dynamic property so QSS attribute selectors can
                // render a pressed state (plain QWidget has no :pressed)
                w->setProperty("_rpPressed", true);
                w->style()->unpolish(w);
                w->style()->polish(w);
            }
            return true; // consume: keeps the release delivered to this widget
        case QEvent::MouseButtonRelease: {
            if (w) {
                w->setProperty("_rpPressed", false);
                w->style()->unpolish(w);
                w->style()->polish(w);
            }
            auto *me = static_cast<QMouseEvent *>(event);
            if (me->button() == Qt::LeftButton && g_hasVoidTrampoline)
                g_voidTrampoline(m_ctx);
            return true;
        }
        default:
...
inline void QWidget_onClicked(QWidget *w, uint64_t ctx) {
    w->installEventFilter(new RpClickEmitter(ctx));
