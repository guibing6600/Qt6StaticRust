...
pub trait AsWidget {
    ...
    /// Show this widget.
    fn show(&self) {
        unsafe { ffi::QWidget_show(self.widget_ptr()); }
    }

    /// Hide this widget.
    fn hide(&self) {
        unsafe { ffi::QWidget_hide(self.widget_ptr()); }
    }

    /// Set the layout direction (0 = LeftToRight, 1 = RightToLeft).
    fn set_layout_direction(&self, dir: i32) {
        unsafe { ffi::QWidget_setLayoutDirection(self.widget_ptr(), dir); }
    }
...
impl Widget {
    ...
    #[doc(hidden)]
    pub fn from_raw(ptr: *mut ffi::QWidget, has_parent: bool) -> Self {
    ...
    /// Connect a click callback to this plain widget.
    ///
    /// `QWidget` has no `clicked()` signal, so this installs a C++ event
    /// filter (`RpClickEmitter`, see `QWidget_onClicked`) that fires on
    /// left-button release and gives the widget a pointing-hand cursor.
    /// The closure handle follows the standard rule: reclaimed on Drop
    /// when the widget has no parent, leaked otherwise (prevents
    /// use-after-free) — for app-lifetime pages this is intentional.
    pub fn connect_clicked<F: Fn()>(&mut self, f: F) {
        debug_assert!(!self.ptr.is_null());
        let handle = crate::signal::leak_void(f);
        unsafe { ffi::QWidget_onClicked(self.ptr, handle.token); }
...
/// `Rc<T>` 共享句柄同样可作为控件加入布局/传参。
///
/// 用途：闭包（信号回调）需要操作控件时，通过 `Rc` 克隆捕获。
/// `set_has_parent` 通过内部指针写入标志位——qtrs 限定 UI 单线程使用，
/// 此写入发生在构建期且无并发访问，实践中安全。
impl<T: AsWidget> AsWidget for std::rc::Rc<T> {
    fn widget_ptr(&self) -> *mut ffi::QWidget {
        (**self).widget_ptr()
    }

    fn set_has_parent(&mut self) {
        let inner: *mut T = std::rc::Rc::as_ptr(self) as *mut T;
        unsafe { (*inner).set_has_parent() }
    }
}
