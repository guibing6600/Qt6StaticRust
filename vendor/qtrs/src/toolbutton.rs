...
impl ToolButton {
    ...
    /// Set auto-exclusive (radio-like) behavior among sibling checkable buttons.
    pub fn set_auto_exclusive(&self, on: bool) {
        debug_assert!(!self.ptr.is_null());
        unsafe { ffi::QToolButton_setAutoExclusive(self.ptr, on); }
    }

    /// Set the icon size in pixels.
    pub fn set_icon_size(&self, w: i32, h: i32) {
        debug_assert!(!self.ptr.is_null());
        unsafe { ffi::QToolButton_setIconSize(self.ptr, w, h); }
    }
